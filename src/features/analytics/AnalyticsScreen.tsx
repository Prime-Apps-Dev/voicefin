...
import { Transaction, SavingsGoal, ExchangeRates, TransactionType } from '../../core/types';
  ...
  const handleGenerateAnalysis = async () => {
    ...
    try {
      const payload = {
        ...
        totalIncome: formatCurrency(periodData.totalIncome, defaultCurrency, language),
        totalExpense: formatCurrency(periodData.totalExpense, defaultCurrency, language),
        netSavings: formatCurrency(periodData.netSavings, defaultCurrency, language),
        topCategories: periodData.topCategories.map(([name, amount]) => [name, formatCurrency(amount, defaultCurrency, language)]),
        totalInSavingsGoals: formatCurrency(savingsGoals.reduce((sum, g) => sum + convertCurrency(g.currentAmount, g.currency, defaultCurrency, rates), 0), defaultCurrency, language)
      };
        ...
        <motion.div className="bg-gray-800/70 backdrop-blur-sm rounded-3xl p-6 border border-gray-700/50" variants={zoomInOut} whileInView="whileInView" viewport={{ once: true }}>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-green-400">{formatCurrency(periodData.totalIncome, defaultCurrency, language)}</span>
            </div>
            <div className="flex items-center justify-between">
              ...
              <span className="font-semibold text-red-400">{formatCurrency(periodData.totalExpense, defaultCurrency, language)}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`font-bold text-lg ${periodData.netSavings >= 0 ? 'text-white' : 'text-red-400'}`}>{formatCurrency(periodData.netSavings, defaultCurrency, language)}</span>
            </div>
          </div>
        </motion.div>
        ...
        <motion.div className="bg-gray-800/70 backdrop-blur-sm rounded-3xl p-6 border border-gray-700/50" variants={zoomInOut} whileInView="whileInView" viewport={{ once: true }}>
          ...
          <AnimatePresence>
            {periodData.topCategories.length > 0 ? (
                  ...
                  return (
                    <div key={category}>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-gray-400">{formatCurrency(amount, defaultCurrency, language)}</span>
                      </div>
              ...
              <div className="text-gray-400 text-center py-4">{t('noData')}</div>
            )}
          </AnimatePresence>
        </motion.div>
